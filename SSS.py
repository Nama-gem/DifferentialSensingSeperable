...
for i in np.arange(0, len(S_)):
    ...
    res = minimize(lambda x: F_prod_der(x[0], oat(x[1], S, Bx), oat(x[1], S, Bx), basis, ind_rho), np.array([phi[i - 1], 0.1915599]), jac = True, method = method,
                  bounds = Bounds([0, 0], [np.pi / 8, 0.25]))
    # mu[np.max([i - 2, 0])]
    # 0.1915599
